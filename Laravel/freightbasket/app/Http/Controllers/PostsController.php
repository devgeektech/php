...
class PostsController extends Controller
{
    ...
    public function fetch(Request $request){
        ...
        $post_min_id = intval($request->input('post_min_id')); // If not empty, timeline_id < post_min_id
        $post_max_id = intval($request->input('post_max_id')); // If not empty, timeline_id > post_max_id
        $div_location = $request->input('div_location');

        $user = Auth::user();

        $posts = Timeline::with('user');
    ...
    public function like(Request $request){
        ...
        if ($post){
            $post_like = PostLike::where('timeline_id', $post->id)->where('like_user_id', $user->id)->get()->first();

            if ($post_like) { // UnLike
                if ($post_like->like_user_id == $user->id) {
                    $deleted = DB::delete('delete from post_likes where timeline_id='.$post_like->timeline_id.' and like_user_id='.$post_like->like_user_id);
                    if ($deleted){
                        $response['code'] = 200;
            ...
            if ($process){
                if ($post->save()) {
                    if ($post->has_image == 1) {
                        ...
                        $post_image->timeline_id = $post->id;
                        if ($post_image->save()){
                            $response['code'] = 200;
                        }else{
                            $response['code'] = 400;
