...
$("#form-new-comment .media-body textarea").keyup(function(event) {
    if (event.keyCode === 13) {
        var pa_classname = $(this).parent().parent().find("button");
        console.log(pa_classname);
        $(pa_classname).click();
    }
});


jQuery('#offerRejectmyModal').on('show.bs.modal', function (event) {
    jQuery(".offer_reject_input").remove();
    var myVal = jQuery(event.relatedTarget).data('val');
    jQuery(".offer_reject .modal-body form").append("<input type='hidden' class='offer_reject_input' name='offer_id' value='"+myVal+"'>");
});