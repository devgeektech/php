<?php
class ControllerExtensionDSEOModuleManagerDSEOModuleBlog extends Controller {
	private $codename = 'd_seo_module_blog';
	private $route = 'extension/d_seo_module_manager/d_seo_module_blog';
	...
	public function manager_config() {
		...
		foreach ($manager_setting['sheet'] as $sheet) {
			if (substr($sheet['name'], 0, strlen('text_')) == 'text_') {
				$manager_setting['sheet'][$sheet['code']]['name'] = $_language->get($sheet['name']);
			}
