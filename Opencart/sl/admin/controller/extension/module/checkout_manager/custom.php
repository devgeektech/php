<?php
class ControllerExtensionModuleCheckoutManagerCustom extends Controller {
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/checkout_manager/custom', 'user_token=' . $this->session->data['user_token'] . $url, true),
            'separator' => ' :: '
        );

        $data['add'] = $this->url->link('extension/module/checkout_manager/custom/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['delete'] = $this->url->link('extension/module/checkout_manager/custom/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);


        $total_custom_fields = $this->model_extension_checkout_manager_checkout->getTotalCustomFields();
        $custom_fields = $this->model_extension_checkout_manager_checkout->getCustomFields();
        ...
        foreach ($custom_fields as $row) {
            // if ($row['field_existance'] != 'default') {
                ...
                $data['custom_fields'][] = array(
                    ...
                    'status'            => $row['status'],
                    'edit'              => $this->url->link('extension/module/checkout_manager/custom/edit', 'user_token=' . $this->session->data['user_token'] . '&field_id=' . $row['db_field_id'] . $url, true)
                );
            // }
        }

        $pagination = new Pagination();
        $pagination->total = $total_custom_fields;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link('extension/module/checkout_manager/custom', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

        $data['pagination'] = $pagination->render();
    
        $data['pageresults'] = sprintf($this->language->get('text_pagination'), ($total_custom_fields) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($total_custom_fields - $this->config->get('config_limit_admin'))) ? $total_custom_fields : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $total_custom_fields, ceil($total_custom_fields / $this->config->get('config_limit_admin')));
        ...
        $this->response->setOutput($this->load->view('extension/module/checkout_manager/custom_fields', $data));
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('extension/module/checkout_manager/custom', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
    ...
    $data['text_filters_conjunction'] = $this->language->get('text_filters_conjunction');
        ...
        if (!isset($this->request->get['field_id'])) {
            $data['action'] = $this->url->link('extension/module/checkout_manager/custom/add', 'user_token=' . $this->session->data['user_token'], true);
        } elseif (isset($this->request->get['field_id'])) {
            $data['action'] = $this->url->link('extension/module/checkout_manager/custom/edit', 'user_token=' . $this->session->data['user_token'] . '&field_id=' . $this->request->get['field_id'], true);
        ...
        $data['cancel'] = $this->url->link('extension/module/checkout_manager/custom', 'user_token=' . $this->session->data['user_token'], true);
    ...
    protected function validateForm() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/checkout_manager/custom')) {
        ...
        if (($field_name == 'country_id') || ($field_name == 'zone_id')) {
            ...
            if ($this->request->post['field_type'] == 'select' || $this->request->post['field_type'] == 'multi-select' || $this->request->post['field_type'] == 'checkbox' || $this->request->post['field_type'] == 'radio') {
        ...
        // check label length
        if (strlen($this->request->post['field_label']) > 255) {
