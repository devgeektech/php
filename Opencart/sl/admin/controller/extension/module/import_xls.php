...
class ControllerExtensionModuleImportXls extends Controller
{
    ...
    public function index(){
        ...
        if(count($this->profiles_select) == 1 && !empty($form))
            $this->session->data['info'] = sprintf($this->language->get('profile_start_to_work'), '<a href="javascript:{}" onclick="$(\'a.tab_export---import, a.tab_Экспорт---Импорт\').click()">' , '</a>');
    ...
    function _get_module_data() {
        ...
        $this->is_ocstore = is_dir(DIR_APPLICATION . 'controller/octeam_tools') || is_dir(DIR_APPLICATION . 'controller/howto') || defined('OPENCARTFORUM_SERVER');

        //In OCStore 3.x, manufacturer hasn't "name" field multilanguage.
        $this->manufacturer_multilanguage = $this->is_ocstore && !$this->is_oc_3x && version_compare(VERSION, '2.2', '>');
    ...
    public function _send_custom_variables_to_view() {
        ...
        $jquery_variables = array(
            ...
            'extension_version' => (int)abs(filter_var($this->language->get('heading_title'), FILTER_SANITIZE_NUMBER_INT)),
    ...
    public function convert_to_innodb() {
        ...
        //<editor-fold desc="Journal 3 index delete">
            $journal_3 = $this->db->query("SELECT count(*) FROM information_schema.TABLES WHERE (TABLE_SCHEMA = '".DB_DATABASE."') AND (TABLE_NAME = '".DB_PREFIX."journal3_product_attribute')");
            if($journal_3->row['count(*)'] > 0) {
                $key_exist = $this->db->query('SHOW INDEX FROM `'.DB_PREFIX.'journal3_product_attribute` WHERE Key_name = "PRIMARY"');
                if($key_exist->num_rows)
                    $this->db->query('ALTER TABLE `'.DB_PREFIX.'journal3_product_attribute` DROP INDEX `PRIMARY`;');
            }
        //</editor-fold>
    ...
    public function _construct_view_form() {
        ...
        $no_libraries = !is_dir(DIR_SYSTEM.'library/Spout') || !is_dir(DIR_SYSTEM.'library/xml2array') || !is_dir(DIR_SYSTEM.'library/google_spreadsheets') || !is_dir(DIR_SYSTEM.'library/PhpSpreadsheet');
    ...
    private function ieProClassLoader( $className) {
        ...
        $filename = "{$ieProDir}/{$className}.php";
