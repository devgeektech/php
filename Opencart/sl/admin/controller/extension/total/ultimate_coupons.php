...
class ControllerExtensionTotalUltimateCoupons extends Controller {
	private $type = 'total';
	...
	public function index() {
		...
		// extension-specific
		$coupon_history_table = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "coupon_history WHERE Field = 'coupon_id'");
		if (strtolower($coupon_history_table->row['Type']) == 'int(11)') {
			$this->db->query("ALTER TABLE " . DB_PREFIX . "coupon_history MODIFY `coupon_id` varchar(255) NOT NULL");
		}
		...
		$data['rule_options'] = array(
			...
			'discount_criteria'			=> array('gift_voucher', 'ignore_specials', 'reward_points'),
			'location_criteria'			=> array('address', 'city', 'country', 'distance', 'geo_zone', 'location_comparison', 'postcode', 'zone'),
			'order_criteria'			=> array('currency', 'custom_field', 'customer_data', 'customer_group', 'language', 'past_orders', 'payment_extension', 'shipping_cost', 'shipping_extension', 'shipping_rate', 'store'),
			'product_criteria'			=> array('attribute', 'attribute_group', 'category', 'manufacturer', 'option', 'product', 'product_group', 'recurring_profile', 'other_product_data'),
		...
		$data['customer_data_array'] = array(
			'email',
			'customer_id',
			'firstname',
			'lastname',
		...
		$data['recurring_profile_array'] = array();
		if (version_compare(VERSION, '2.0', '>=')) {
			$this->load->model('catalog/recurring');
			foreach ($this->model_catalog_recurring->getRecurrings() as $profile) {
				$data['recurring_profile_array'][$profile['recurring_id']] = $profile['name'];
			}
		}
		...
		$data['settings'][] = array(
			'type'		=> 'tabs',
			'tabs'		=> array('extension_settings', 'coupons', 'product_groups', 'rule_sets', 'testing_mode'),
		);
		$data['settings'][] = array(
		...
		$data['settings'][] = array(
			'type'		=> 'html',
			'content'	=> '<div class="text-info text-center pad-bottom">' . $data['help_coupons'] . '</div>',
		);
		$data['settings'][] = array(
			'key'		=> 'coupons',
			'type'		=> 'heading',
		...
		$data['settings'][] = array(
			'key'		=> $table,
			'type'		=> 'table_start',
			'columns'	=> array('action', 'coupon', 'title', 'charge', 'rules'),
		);
		foreach ($this->getTableRowNumbers($data, $table, $sortby) as $num => $rules) {
			...
			$report_code = (isset($data['saved']['charge_' . $num . '_coupon'])) ? $data['saved']['charge_' . $num . '_coupon'] : '';
			$report_link = $this->url->link('extension/' . $this->type . '/' . $this->name . '/report', 'code=' . $report_code . '&token=' . $data['token'], 'SSL');
			
			$data['settings'][] = array(
				'type'		=> 'html',
				'content'	=> '
					<a target="_blank" class="btn btn-info" data-help="' . $data['text_view_report'] . '" href="' . $report_link . '">
			...
			$data['settings'][] = array(
				'key'		=> $prefix . 'coupon',
				'type'		=> 'text',
				'before'	=> $data['text_code'],
				'attributes'=> array('style' => 'font-family: monospace'),
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'uses_per_coupon',
				'type'		=> 'text',
				'class'		=> 'short',
				'before'	=> $data['text_uses_per_coupon'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'uses_per_customer',
				'type'		=> 'text',
				'class'		=> 'short',
				'before'	=> $data['text_uses_per_customer'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'status',
				'type'		=> 'select',
	...
	public function report() {
		...
		$coupon_history_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_history WHERE coupon_id = '" . $this->db->escape($this->request->get['code']) . "'");
		
		$html = '<table border="1" style="font-family: monospace; width: 100%" cellspacing="0" cellpadding="5">';
		$html .= '<tr><td><b>Order ID</b></td><td><b>Customer</b></td><td><b>Amount</b></td><td><b>Date Added</b></td></tr>';
		
		if (!$coupon_history_query->num_rows) {
			$html .= '<tr><td colspan="4">No coupon history</td></tr>';
		} else {
			$token = (version_compare(VERSION, '3.0', '<')) ? 'token=' . $this->request->get['token'] : 'user_token=' . $this->request->get['user_token'];
			
			foreach ($coupon_history_query->rows as $row) {
				$order_link = $this->url->link('sale/order/info', 'order_id=' . $row['order_id'] . '&' . $token, 'SSL');
				$order = '<a href="' . $order_link . '">' . $row['order_id'] . '</a>';
				
				$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = " . (int)$row['customer_id']);
				if ($customer_query->num_rows) {
					$customer_link = $this->url->link((version_compare(VERSION, '2.1', '<') ? 'sale' : 'customer') . '/customer/edit', 'customer_id=' . $row['customer_id'] . '&' . $token, 'SSL');
					$customer = '<a href="' . $customer_link . '">' . $customer_query->row['firstname'] . ' ' . $customer_query->row['lastname'] . '</a>';
				} else {
					$customer = 'Guest';
				}
				...
				$html .= '<td>' . $order . '</td>';
				$html .= '<td>' . $customer . '</td>';
				$html .= '<td>' . $this->currency->format(-$row['amount'], $this->config->get('config_currency')) . '</td>';
				$html .= '<td>' . $row['date_added'] . '</td>';
				$html .= '</tr>';
			}
			
			$coupon_totals_query = $this->db->query("SELECT COUNT(*) AS orders, COUNT(DISTINCT customer_id) AS customers, SUM(amount) AS total FROM " . DB_PREFIX . "coupon_history WHERE coupon_id = '" . $this->db->escape($this->request->get['code']) . "'");
			
			$html .= '<tr>';
			$html .= '<td align="right"><b>Total Orders: ' . $coupon_totals_query->row['orders'] . '</b></td>';
			$html .= '<td align="right"><b>Total Customers: ' . $coupon_totals_query->row['customers'] . '</b></td>';
			$html .= '<td align="right"><b>Total Amount: ' . $this->currency->format(-$coupon_totals_query->row['total'], $this->config->get('config_currency')) . '</b></td>';
			$html .= '<td></td>';
			$html .= '</tr>';
		}
