<?php
    class ModelExtensionModuleIeProExport extends ModelExtensionModuleIePro {
        ...
        private function is_comparator( $value){
            if (is_string( $value))
            {
                ...
                $result = in_array( $value, ['>', '<', '>=', '<=', '=', '!=',
                                             'LIKE', 'NOT_LIKE',
                                             'YEARS_AGO', 'MONTHS_AGO', 'DAYS_AGO', 'HOURS_AGO', 'MINUTES_AGO']);
            }
            else
            {
                ...
                foreach ($this->columns as $key => $col_info) {
                    ...
                    if ($result >= 1)
                    {
                      foreach ($matches[1] as $fieldName)
                      {
                        ...
                        if (!empty( $fieldName) && isset( $el[$fieldName]))
                        {
                          $default_value = str_replace( "[{$fieldName}]", $this->conversion_value($fieldName, $el[$fieldName]), $default_value);
                        }
                      }
                    }
            ...
            if($col_info) {
                if(array_key_exists('html_tags', $col_info) && !empty( trim( $col_info['html_tags']))) {
                    if($col_info['html_tags'] == 'all')
                        $final_value = strip_tags($final_value);
                    else
                            ...
                            if ($get_value_from_column) {
                                if ($this->is_string_expression( $final_value)) {
                                    ...
                                    foreach ($matches[1] as $key_match => $column_name) {
                                        ...
                                        $final_value = array_key_exists($column_name, $element) ? $element[$column_name] : 0;
        ...
        private function sort_attributes( $rows, $attributeColumns) {
            ...
            $sortedValues = $this->get_attributes_sorted_values( $rows, $attributeColumns);
            $sortedAttrColumns = $this->build_sorted_attribute_columns( $attributeColumns);
            $sortedAttrColumnsCount = count( $sortedAttrColumns);

            foreach ($rows as $row) {
                $rowAttrValues = $this->get_row_attribute_values( $row, $sortedAttrColumns);
                $sortedAttrValues = $this->get_sorted_attribute_values( $rowAttrValues, $sortedValues);

                $attrColumns = $sortedAttrColumns;
                $sortedAttrValuesCount = count( $sortedAttrValues);

                if ($sortedAttrValuesCount > $sortedAttrColumnsCount) {
                    $sortedAttrValues = array_slice( $sortedAttrValues, 0, $sortedAttrColumnsCount);
                } elseif ($sortedAttrValuesCount < $sortedAttrColumnsCount) {
                    $attrColumns = array_slice( $attrColumns, 0, $sortedAttrValuesCount);
                }

                foreach ($sortedAttrValues as $index => $values) {
                    $columnSet = $attrColumns[$index];

                    $groupColumn = $columnSet['group']['custom_name'];
                    $nameColumn = $columnSet['name']['custom_name'];
                    $valueColumn = $columnSet['value']['custom_name'];

                    $row[$groupColumn] = $values['group'];
                    $row[$nameColumn] = $values['name'];
                    $row[$valueColumn] = $values['value'];
                }
        ...
        private function get_attributes_sorted_values( $rows, $attributeColumns) {
            $values = [];

            foreach ($rows as $row) {
                foreach ($attributeColumns as $attributeColumn) {
                    if ($attributeColumn['field'] === 'attribute') {
                        $columnName = $attributeColumn['custom_name'];
        ...
        private function build_sorted_attribute_columns( $attributeColumns) {
            $result = [];

            foreach ($attributeColumns as $attributeColumn) {
                $identificator = $attributeColumn['identificator'];
                $idParts = preg_split( '/_/', $identificator);
                $position = +$idParts[0] - 1;
                ...
                switch ($attributeColumn['field'])
                {
                    case 'attribute_group':
                        $result[$position]['group'] = $attributeColumn;
                        break;

                    case 'attribute':
                        $result[$position]['name'] = $attributeColumn;
                        break;

                    case 'attribute_value':
                        $result[$position]['value'] = $attributeColumn;
                        break;
                }
            }
        ...
        private function get_row_attribute_values( $row, $attributeColumns) {
            $result = [];

            foreach ($attributeColumns as $attributeColumn) {
                $groupColumn = $attributeColumn['group']['custom_name'];
                $nameColumn = $attributeColumn['name']['custom_name'];
                $valueColumn = $attributeColumn['value']['custom_name'];

                $result[] = [
                    'group' => $this->safe_value( $row, $groupColumn),
                    'name' => $this->safe_value( $row, $nameColumn),
                    'value' => $this->safe_value( $row, $valueColumn)
                ];
            }
        ...
        private function get_sorted_attribute_values( $rowAttrValues, $sortedValues) {
            $result = [];

            foreach ($sortedValues as $value) {
               $item = null;

               foreach ($rowAttrValues as $rowAttrValue) {
                   if ($rowAttrValue['name'] === $value) {
                      $item = $rowAttrValue;
                      break;
                   }
               }
        ...
        private function safe_value( $array, $index, $default = '') {
        ...
        private function find_option_value( $name, $id, $elements) {
            ...
            if ($option_column !== null) {
                foreach ($elements as $key => $element) {
                    if ($this->starts_with( $key, "{$id}_option") &&
                        isset( $element[$name])) {
                        $result = $element[$name];
                        break;
                    }
