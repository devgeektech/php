    ...
    class ModelExtensionModuleIeProProducts extends ModelExtensionModuleIePro
    {
        ...
        function get_columns_formatted($multilanguage)
        {
            $fields = array(
                ...
                'Main category' => array('hidden_fields' => array('table' => 'product_to_category', 'field' => 'main_category', 'allow_ids' => true)),
...
//                Check if it has options combinations columns
                ...
                if ($this->hasOptionsCombinations && !$is_option_combination_row){
            ...
            //Options combinations - Assign main "Quantity" product like sum of all options combinations quantity
            if($this->hasOptionsCombinations) {
                foreach ($copy_data_file as $key => $pro) {
                    $hasOptionsCombination = !empty($pro['product_options_combinations']);
                    if(array_key_exists('quantity', $pro['product']) && $hasOptionsCombination) {
                        $quantity = 0;
                        foreach ($pro['product_options_combinations'] as $comb)
                            $quantity += !empty($comb['quantity']) ? $comb['quantity'] : 0;

                        $copy_data_file[$key]['product']['quantity'] = $quantity;
                    }

                    //Fix bug with prices + strict update enabled
                    if($this->strict_update && $hasOptionsCombination) {
        ...
        public function get_product_by_key() {
            $results = $this->db->query("SELECT ".$this->escape_database_field('product_id').', '.$this->escape_database_field($this->product_identificator).' FROM '.$this->escape_database_table_name('product'));
            $final_result = array();
            if($results->num_rows) {
                foreach ($results->rows as $key => $prod) {
                    $final_result[$prod[$this->product_identificator]] = $prod['product_id'];
                }
            }
            return $final_result;
        }
        ...
        function assign_product_id($force_id = false) {
            if(!$force_id || ($force_id && (!is_numeric($force_id) || is_float($force_id)))) {
                $this->db->query('INSERT INTO ' . $this->escape_database_table_name('product') . ' SET ' . $this->escape_database_field('model') . ' = ""');
        ...
        public function get_product_option_values($product_id) {
            $sql = 'SELECT pov.*, ov.sort_order FROM '.$this->escape_database_table_name('product_option_value').' pov LEFT JOIN '.$this->escape_database_table_name('option_value').' ov ON(pov.option_value_id = ov.option_value_id) WHERE product_id = '.$this->escape_database_value($product_id);
        ...
        public function check_file_option_column_keys($data_file, $force_names = false) {
            ...
            foreach ($data_file['columns'] as $key => $col_name) {
                ...
                if($col_info) {
                    $table = array_key_exists('table', $col_info) ? $col_info['table'] : '';
            ...
            if(!empty($attributes) && is_array($attributes)) {
                foreach ($attributes as $key => $element) {
                    ...
                    foreach ($attribute_values as $lang_id => $attri_val_name) {
                        if(!empty($attr_group_name) && !empty($attri_val_name)) {
                            $temp = array(
                                'product_id' => $product_id,
                            ...
                            if(!$this->multilanguage && $this->count_languages_real > 1) {
        ...
        public function _importing_process_format_product_related($related, $product_id, $row_file_number) {
            ...
            foreach ($related as $key => $model) {
                if(!empty($model)) {
                    $model = trim($model);
                    $related_id = $this->get_product_id($this->related_identificator, $model);
            ...
            if(!empty($specials) && is_array($specials)) {
                foreach ($specials as $number => $special_datas) {
                    foreach ($special_datas as $customer_group_id => $special_data) {
                        ...
                        if($some_special_data && $price_exist) {
            ...
            if(!empty($discounts) && is_array($discounts)) {
                foreach ($discounts as $number => $discount_datas) {
                    foreach ($discount_datas as $customer_group_id => $discount_data) {
                        ...
                        $price_exist = array_key_exists('price', $discount_data) && !empty($discount_data['price']);
            ...
            foreach ($option_cmb_values as $table_name => $value) {
                if ($table_name == 'options') {
                    ...
                    foreach ($value as $option_and_value) {
                        ...
                        //if there isn't one of the option_values or $option_id then the combination being proccesed its skipped because this
                        //could break the combination module.
                        if ((empty($option_name) || empty($option_value_name))) {
                            continue;
                        }
                        ...
                        if ($option_id && !$this->exist_option_id($option_id)){
                            return [];
                        }
                        ...
                        if (!$option_id) {
                            $option_id = (string) $this->model_extension_module_ie_pro_options->create_simple_option([
                                'name' => [$option_name_language_id => $option_name],
                                'type' => $option_type
                            ]);
                        ...
                        elseif (!$option_value_id && !$this->its_an_option_value_list($option_value_name)){
                            ...
                            if (!$option_value_id) {
                                $option_value_id = $this->model_extension_module_ie_pro_option_values->create_simple_option_value([
                                    'name' => [$option_value_name_language_id => $option_value_name],
                                    'option_id' => $option_id,
                                    'sort_order' => 0
                                ]);
//                                $new_key = "{$option_id}_{$option_value_name}_0";
//                                $this->all_option_values[$new_key] = $option_value_id;
                            }
                        }
                        elseif (!$option_value_id && $this->its_an_option_value_list($option_value_name)){
...
//                                    $this->all_option_values[$new_key] = $id;
                    ...
                    foreach ($images as $key => $image){
                        if ($this->is_url($image)) {
                            $force_name = 'comb-'.$row_number.'-'.$count_comb_image;
                            $count_comb_image++;
                            $final_images[$key] = $this->download_remote_image($table_name, $product_id, $row_number, $image, $force_name);
                        } else {
                    ...
                    if (array_key_exists('option_special', $value)) {
                        foreach ($value['option_special'] as $option_special) {
                            //ckeck if its a option combination empty row
                            $is_empty_row = empty(array_values($option_special['customer_group_id'])[0])
                            ...
                            $group_name = array_values($option_special['customer_group_id'])[0];
                            $group_name_language_id = array_keys($option_special['customer_group_id'])[0];
                            $group_id = '';
                        ...
                        if($col_info['field'] == 'main_category') {
                            ...
                            if($main_category_id) {
                                ...
                                if($allow_ids)
                                    ...
                                    $current_data[$col_info['custom_name']] = array_key_exists($main_category_id, $this->all_categories) && array_key_exists($this->default_language_id, $this->all_categories[$main_category_id]['name']) ? $this->all_categories[$main_category_id]['name'][$this->default_language_id] : '';
                            }
                            continue;
                        }
                        $allow_ids = $this->check_allow_ids($col_info);
        ...
        public function _exporting_process_product_options_combinations($product_id, $columns) {
            $product_cmbs_values = $this->get_product_option_cmb_values($product_id);
            $product_cmbs = array();
            if (!empty($product_cmbs_values)) {
                foreach ($product_cmbs_values as $product_cmbs_value) {
                    $product_cmb = array();
                    foreach ($columns as $col_name => $col_info) {
                        if (isset($col_info['data_type']) && $col_info['data_type'] == 'json') {
                            $json_value = json_decode($product_cmbs_value[$col_info['field']], true);
                            if ($col_info['field'] == 'options' && is_array($json_value)) {
                                $index = explode('_', $col_info['identificator'])[0];
                                $count = 1;
                                foreach ($json_value as $key => $value) {
                                    if ($count == $index) {
                                        if ($col_info['inner_field'] == 'option') {
                                            if (isset($col_info['id_instead_of_name']) && $col_info['id_instead_of_name'] == 1){
                                                $product_cmb[$col_info['custom_name']] = $key;
                                            }
                                            else{
                                                //If value is an array, thats the case when it is an ckeckbox option,  then its necessary to extract the first value and get its option value
                                                $option_value = (is_array($value)) ? $this->all_option_values[$value[0]] : $this->all_option_values[$value];
                                                $product_cmb[$col_info['custom_name']] = (array_key_exists('language_id', $col_info)) ?
                                                    $option_value['option']['name'][$col_info['language_id']] :
                                                    $option_value['option']['name'];
                                            }
                                            break;
                                        } elseif ($col_info['inner_field'] == 'option_value') {
                                            if (isset($col_info['id_instead_of_name']) && $col_info['id_instead_of_name'] == 1){
                                                if (is_array($value)) $value = json_encode(array_values($value));
                                                $product_cmb[$col_info['custom_name']] = $value;
                                            }
                                            else{
                                                if (is_array($value)){
                                                    $arr_values = [];
                                                    foreach ($value as $option_value_id){
                                                        ...
                                                        $arr_values[] = (array_key_exists('language_id', $col_info)) ?
                                                            $option_value['name'][$col_info['language_id']] :
                                                            $option_value['name'];
                                                    }
                                                    $product_cmb[$col_info['custom_name']] = json_encode(array_values($arr_values));

                                                } else {
                                                    $option_value = $this->all_option_values[$value];
                                        ...
                                        elseif ($col_info['inner_field'] == 'option_type'){
                                            $option_value = (is_array($value)) ? $this->all_option_values[$value[0]] : $this->all_option_values[$value];
                                            $product_cmb[$col_info['custom_name']] = $option_value['option']['type'];
                                        }
                                    }
                                    $count++;
                                }
                            ...
                            elseif ($col_info['field'] == 'prices' && ($col_info['inner_field'] == 'option_price' || $col_info['inner_field'] == 'option_points')){
                                $inner_field = $col_info['inner_field'];
                                $option_cmb_values = $json_value[$inner_field];
                                ...
                                foreach ($option_cmb_values as $option_cmb_value){
                                    if (!array_key_exists('customer_group_id', $option_cmb_value) || $col_info['customer_group_id'] == $option_cmb_value['customer_group_id']){
                                        $product_cmb[$col_info['custom_name']] = $option_cmb_value[$col_info['key']];
                                    }
                                }
                            }
                                ...
                                foreach ($option_cmb_values as $index => $option_cmb_value){
                                    if ($index == $identificator - 1){
                                        if ($col_info['key'] == 'customer_group_id' && !isset($col_info['id_instead_of_name'])){
                                            ...
                                            $product_cmb[$col_info['custom_name']] = count($customer_group) != 0 ? array_pop($customer_group)['name'] : '';
                                        }
                                        else{
                                            $product_cmb[$col_info['custom_name']] = array_key_exists($col_info['key'], $option_cmb_value) ? $option_cmb_value[$col_info['key']] : '';
                                        }
                                    }
                                }
                                ...
                                if (!empty($json_value) && $json_value != '') {
                                    foreach ($json_value as $index => $image_value) {
                                        if ($col_info['identificator'] - 1 == $index) {
                                                $product_cmb[$col_info['custom_name']] = $image_value;
                                        }
                                    }
                                }
                            }
                        } else {
                            $product_cmb[$col_info['custom_name']] = $product_cmbs_value[$col_info['field']];
                        }
                    }
                    if (!empty($product_cmb))
                        $product_cmbs[] = $product_cmb;
                }
            }
            return $product_cmbs;
        }

        private function get_product_option_cmb_values($product_id){
            $option_cmbs = $this->db->query('SELECT * FROM '.$this->escape_database_table_name('product_options_combinations').' WHERE product_id = '.$this->escape_database_value($product_id));
            $final_option_cmbs_values = array();
            foreach ($option_cmbs->rows as $key => $val) {
                $final_option_cmbs_values[] = $val;
            }
            return $final_option_cmbs_values;
        }
