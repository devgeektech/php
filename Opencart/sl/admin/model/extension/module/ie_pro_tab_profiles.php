...
//                            'after' => $this->get_remodal('profile_opt_cmb_images_number', $this->language->get('profile_options_combinations_images_number_remodal_title'), $this->language->get('profile_options_combinations_images_number_remodal_description'), array('button_cancel' => false, 'button_confirm' => false, 'link' => 'profile_import_special_number_link')),
        ...
        public function get_profile_categories_mapping_columns_html(){
            ...
            try
            {
               ...
               return $categories_analyzer->get_result();
            }
            catch (\Exception $ex) {
                $this->output_error( $ex->getMessage());
            }
        ...
        public function get_profile_filters_html($database_fields, $is_an_import_profile = false, $element_type = null) {
            ...
            $button_add_filter = '<a href="javascript:{}" onclick="profile_add_filter($(this));" class="button" title="'.$this->language->get('profile_products_filters_add_filter').'" style="padding: 5px 12px;"><i class="fa fa-plus-square" aria-hidden="true"></i></a>';

            if (!$is_an_import_profile)
            {
              $button_open_group = '<a href="javascript:{}" onclick="profile_export_filter_open_group($(this));" class="button open_group disabled" title="'.$this->language->get('profile_products_filters_open_group').'" style="padding: 5px 12px; margin-left: 5px;"><i class="fa fa-indent" aria-hidden="true"></i></a>';
              $button_close_group = '<a href="javascript:{}" onclick="profile_export_filter_close_group($(this));" class="button close_group disabled" title="'.$this->language->get('profile_products_filters_close_group').'" style="padding: 5px 12px; margin-left: 5px;"><i class="fa fa-dedent" aria-hidden="true"></i></a>';
            }

            $button_remove_filter = '<a href="javascript:{}" onclick="profile_remove_filter($(this));" class="button danger" title="'.$this->language->get('profile_products_filters_remove_filter').'" style="padding: 5px 12px;"><i class="fa fa-minus-square" aria-hidden="true"></i></a>';
            ...
            if (!$is_an_import_profile)
            {
                ...
                $html .= '<script>window._INITIAL_FILTERS = ' . json_encode( $filter_config) . '; profile_export_build_filters_table();</script>';
            }
        ...
        function get_filter_row($number, $fields_to_select, $filter_config = array(), $button_remove_filter, $is_model = false, $filter_number = false, $is_an_import_profile = false, $element_type = null) {
            ...
            if ($is_an_import_profile){
                ...
                if ($element_type == 'products') $actions_to_select['disable'] = $this->language->get('profile_products_filters_actions_disable');
        ...
        function get_custom_column_fixed_row($number, $custom_columns_fixed_config = array(), $button_remove, $is_model = false, $columns_number = false) {
