...
class ModelExtensionModuleWarehouse extends Model {
	...
	public function getProductsForCsv($data = array()) {
		...
		$sql = "SELECT DISTINCT pt.product_id,p.name AS productname,p.product_id as id FROM " . DB_PREFIX . "product_description p  LEFT JOIN " . DB_PREFIX . "product pt ON (pt.product_id = p.product_id) ";
