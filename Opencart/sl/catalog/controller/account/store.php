...
class ControllerAccountStore extends Controller {
    ...
    public function index() {
        ...
        $data['is_suggested_product_found'] = 0;
        ...
        if (empty($man_product['found_in_past_order'])) {
            if (!empty($man_product['mandatory_product_id'])) {
                ...
                $data['is_suggested_product_found'] = 1;
            } else {
    ...
    // redirect mandatory share product from below funtion
    ...
    // redirect suggested share product from below funtion
    public function additional_shares() {
        $this->load->model('catalog/product');
        $harvest_id = $this->customer->harvestId();
        $man_product = $this->model_catalog_product->satisfied_mandatory_purchases_all($harvest_id);
        if (!empty($man_product['suggested_product_id'])) {
