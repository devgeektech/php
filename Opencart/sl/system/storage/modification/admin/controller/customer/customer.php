...
class ControllerCustomerCustomer extends Controller {
	...
	public function history() {
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
        		...
        		'notify'    => $this->language->get($result['notified'] ? 'text_yes' : 'text_no'),
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added']))
			);
		}
	...
	public function addHistory() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
			...
			$history_notify = !empty($this->request->post['history_notify']) ? true : false;

                $this->model_customer_customer->addHistory($this->request->get['customer_id'], $this->request->post['comment'], $history_notify);
