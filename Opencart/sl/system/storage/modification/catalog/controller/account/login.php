...
// Clear Thinking: MailChimp Integration Pro
			...
			} elseif (isset($this->request->session['login_redirect']) && (strpos($this->request->session['login_redirect'], $this->config->get('config_url')) !== false)) {
				$this->response->redirect(str_replace('&amp;', '&', $this->request->session['login_redirect']));
			} else {
				$this->response->redirect($this->url->link('account/store', '', true));
			}
		}
