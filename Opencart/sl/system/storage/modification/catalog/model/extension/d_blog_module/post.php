...
class ModelExtensionDBlogModulePost extends Model
{
    ...
    public function updateViewed($post_id)
    {
        $this->db->query("UPDATE " . DB_PREFIX . "bm_post SET viewed = (viewed + 1) WHERE post_id = '" . (int)$post_id . "'");
                ...
                public function gettypePosts($data = array())
                {
                    ...
                    $sql .= " LEFT JOIN " . DB_PREFIX . "bm_post_description pd ON (p.post_id = pd.post_id) LEFT JOIN " . DB_PREFIX . "d_meta_data md ON (md.route = CONCAT('bm_post_id=', p.post_id) AND md.store_id = '" . (int)$this->config->get('config_store_id') . "' AND md.language_id = '" . (int)$this->config->get('config_language_id') . "') "
    ...
    {
        $sql = "SELECT p.post_id, p.user_id, p.type, p.limit_access_user,p.limit_users,p.limit_access_user_group,p.limit_user_groups,p.user_id, p.image,p.image_title,p.image_alt, p.images_review, pd.tag, pd.title, pd.meta_title, p.date_added, p.date_modified, p.date_published, p.review_display, p.viewed,   "
            . "pd.meta_description, pd.meta_keyword, pd.description, "
