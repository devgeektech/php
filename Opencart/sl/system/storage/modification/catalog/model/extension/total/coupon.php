...
// Clear Thinking: Ultimate Coupons
				...
				if ($this->config->get($prefix . 'ultimate_coupons_status')) {
					$this->load->model('extension/total/ultimate_coupons');
					$valid = $this->model_extension_total_ultimate_coupons->checkCoupon($code);
					if ($valid) {
						if ($this->config->get($prefix . 'ultimate_coupons_unset_shipping')) {
...
// Clear Thinking: Ultimate Coupons
				$prefix = (version_compare(VERSION, '3.0', '<')) ? '' : 'total_';
				if ($this->config->get($prefix . 'ultimate_coupons_status')) {
					$code = (isset($this->session->data['coupon'])) ? $this->session->data['coupon'] : '';
					$valid = false;
